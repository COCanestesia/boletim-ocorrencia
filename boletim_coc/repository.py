...
def create_boletim(
    ...
    draft.validate()
    author = windows_user.strip() if author_name is None else str(author_name).strip()
    if not author:
...
def update_draft(conn, boletim_id: str, changes: dict):
    ...
    if "autor_original" in clean:
        clean["autor_original"] = str(clean["autor_original"]).strip()
        if not clean["autor_original"]:
            raise ValueError("Informe o nome do autor.")
    if not clean:
        return row
